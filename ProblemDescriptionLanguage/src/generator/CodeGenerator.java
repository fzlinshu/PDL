...
public class CodeGenerator {
				...
				if (!used.contains(v))
					if (v.ready(used, indeps, var)) {
						if (!v.getName().startsWith("_sum"))
							// TODO other accumulative functions
	...
	public void generate(File file, VarTable varTable, ArrayList<ExpTreeNode> _expTrees, String memlimit)
		...
		printer.print("#include<cstdlib>\n");
		printer.print("#include<set>\n");
		printer.print("\n");
		...
		for (VarRecord v : dependVars)
			if (!v.getName().startsWith("_sum"))
				// TODO other accmulative functions
				printer.print(v.getDef() + ";\n");
		...
		if (!inputVars.isEmpty()) {
			...
			for (VarRecord v : inputVars) {
				...
				int m = calcs.size() - 1;
				calcs.set(m, "std::cin >> " + v.getName() + calcs.get(m) + ";");
				for (String s : calcs) {
					printer.incIndent();
					printer.print(s + "\n");
				}
		...
		for (VarRecord v : outputVars) {
			...
			if (defExp == null) {
				...
				if (obj != Objective.SAT)
					prefix = "_best_";
				calcs.add("std::cout << " + prefix + v.getName() + " << std::endl;");
			} else {
				...
				if (obj != Objective.SAT)
					prefix = "_best_";
				if (m == 0)
					calcs.set(m, "std::cout << " + prefix + v.getName() + calcs.get(m) + " << std::endl;");
				else if (defExp.getOp() != Operator.SET)
					calcs.set(m, "std::cout << " + prefix + v.getName() + calcs.get(m) + " << \" \";");
				else
			...
			for (int i = independArrays.size() - 1; i >= 0; i--) {
				...
				if (next == null)
					...
					for (int c = 0; c < preCount; c++)
						printer.append(", 0");
					// TODO deal with all accumulative functions
					printer.append(");\n");
				}
				if (useDP)
				...
				for (String tmp : tmpnames)
					printer.append(", " + tmp);
				// TODO deal with other accumulative functions
				printer.append(")");
				if (useDP)
		...
		if (obj == Objective.SAT)
			printer.print("std::cout << \"No Solution!\" << std::endl;\n");
		else
			printer.print("_output();\n");
